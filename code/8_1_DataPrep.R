...
# Use spThin to spatially thin data set to achieve a more uniform distribution
sg_records <- read_csv("data/csv/eelgrass_occurrence_records_20230918.csv") %>%
  dplyr::select(source, id, latitude, longitude) %>%
  mutate(species = rep("eelgrass", 8201)) # adding column for species name as required by spThin
head(sg_records)
...
str(thinned) # identify the dataset (i.e. list element) with the largest number of retained observations (should be 1)
# 1118
...
write.csv(thinned2, "output/thinned_locations_200m.csv")
