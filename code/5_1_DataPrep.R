...
# Playing around with spThin
library(spThin)

sg_records <- read_csv("data/csv/eelgrass_occurrence_records_20230918.csv") %>%
  mutate(., species = rep("eelgrass", 8201)) %>%
  dplyr::filter(!is.na(subObs_JT)) %>%
  dplyr::filter(!is.na(latitude) & !is.na(longitude)) %>%
  dplyr::select(source, id, latitude, longitude, species)
head(sg_records)

thinned <-
  thin(loc.data = sg_records,
...
thinned2 <- as.data.frame(thinned) %>%
...
sg_records <- left_join(thinned2, sg_records, by = "id") %>% 
...
str(myBiomodData) # 646

# Save biomod2 formatted data
saveRDS(myBiomodData, file = "data/rds/4_biomod2_data.rds")
...
head(DataSplitTable)
dim(DataSplitTable) # 646

# Save CV partition
saveRDS(DataSplitTable, file = "data/rds/4_CV_folds_biomod.rds")
